<template>
  <div class="function_module" :class="{ hide: datas.is_hide }">
    <div class="module-container">
      ...
      <div class="option-wrap" v-if="datas.is_option == 1">
        <div
          class="option-item"
          :class="select_index(index)"
          v-for="(item, index) in datas.option_data.split('^')"
          :key="index"
          @click="clickoption(index, item)"
        >
      ...
      <div class="module-header" v-if="datas.county_condition">
        ...
        <div
          class="select-group"
          v-for="(aitem, index) of getsxData"
          :key="index"
        >
          <select v-model="other" v-if="aitem.select_value">
            <option
              :value="item"
              v-for="(item, index) of aitem.select_value.split('^')"
              :key="index"
            >
              {{ item }}
            </option>
      ...
      >
        <div class="top-wrap">
          ...
          <div class="network-center">
            <!-- <div
              class="el-icon-arrow-down foldIcon"
              @click="showfold"
              v-if="datas.is_fold == 1 && !showfolds"
            ></div>
            <div
              class="el-icon-arrow-up foldIcon"
              @click="showfold"
              v-if="datas.is_fold == 1 && showfolds"
            ></div> -->
            <div class="header">
              <span class="title">{{ item.networkName }}</span>
              <!-- <span :class="['type', styleArr[3], styleArr[0]]">
                {{ item.itemType }}
              </span> -->
            </div>
            ...
            <ul class="container">
              <li
                class="item"
                v-for="(content, index) of demoData"
                :key="index"
              >
                <template v-if="content.type == 9">
                  <!-- <span class="title">{{ content.name }}</span> -->
                  <span class="text" :class="['search', styleArr[8]]">
                        <span :class="['type', styleArr[3], styleArr[0]]">
                            {{ content.value }}
                        </span>
          ...
          <div class="network-right" v-if="btnArr.length>0">
              <div class="btn-group" style="justifycontent: space-between">
          <button
            v-for="(btn, index) of btnArr"
            :key="index"
            :class="['btn',styleArr[3],styleArr[0],styleArr[7]]"
            :style="showWidth"
          >
            {{ btn.name }}
          </button>
      ...
      <footer v-if="+datas.fold">
        <div
          :class="['showMore', styleArr[0]]"
          v-if="networkList.length > +datas.fold && +datas.fold"
...
export default {
  ...
  computed: {
    ...
    // 拆分datas中文本内容
    contentArr() {
      return this.datas.value_list.filter(
        (item) =>
          !item.default && (item.type == 0 || item.type == 8 || item.type == 9)
      )
    },
    // 拆分datas中btn按钮
    btnArr() {
      return this.datas.value_list.filter(
        (item) => !item.default && (item.type == 5 || item.type == 6)
      )
    },
    others() {
...
.function_module {
  ...
  .module-container {
    ...
    .module-header::-webkit-scrollbar {
      width: 0 !important;
    }
    .module-header {
      color: #000;
