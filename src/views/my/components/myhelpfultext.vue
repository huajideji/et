    ...
    <div class="helpCenter">
        ...
        <hr />
                <ul class="tab-item flex wrap" v-show="tabIndex === 0">
                    <li v-for="(item,index) in helpData.data" :key="index" @mouseleave="hideOperate(0,index)" @click="openUrl(item.url,item.id)">
                    ...
                    <div class="nodata" v-show="helpData.data.length == 0">暂无数据</div>
                </ul>
        <!-- 上传文档的弹窗 -->
        <el-dialog
...
export default {
    ...
    methods: {
        ...
        async search() { 
            ...
            // }
            // const res = await this.$api.getmyhelpList(params)
            // if (res.data_count == 0) {
            //     this.fileList = [[],[],[],[]]
            // }else {
